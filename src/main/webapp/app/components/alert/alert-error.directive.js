(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('homeRecipesApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-homeRecipesApp-error');
                var entityKey = httpResponse.headers('X-homeRecipesApp-params');
                if (errorHeader) {
